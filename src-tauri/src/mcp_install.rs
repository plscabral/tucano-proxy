...
fn client_path_in(home: &Path, c: McpClient) -> PathBuf {
    match c {
        ...
        McpClient::OpenCode | McpClient::OpenCodeDesktop => home.join(".config/opencode/opencode.json"),
...
fn is_installed(c: McpClient, path: &Path) -> bool {
    ...
    match c {
        McpClient::Pi => path.exists() && path.with_file_name("tucano-tools.json").exists() && path.with_file_name("config.json").exists(),
        McpClient::ClaudeDesktop | McpClient::ClaudeCode | McpClient::Gemini | McpClient::Antigravity | McpClient::OhMyPi => {
...
pub fn refresh_installed(settings: &McpSettings) -> Result<(), String> {
    for c in CLIENTS {
        ...
        if matches!(c, McpClient::CodexDesktop | McpClient::OpenCodeDesktop) { continue; }
...
fn install_at(c: McpClient, path: &Path, settings: &McpSettings) -> Result<(), String> {
    ...
    match c {
        McpClient::Pi => {
            ...
            std::fs::write(path, include_str!("../../pi-extension/tucano.ts")).map_err(|e| e.to_string())?;
            write_json(&dir.join("tucano-tools.json"), &crate::mcp_bridge::tools_list())?;
        ...
        McpClient::OpenCode | McpClient::OpenCodeDesktop => {
            ...
            mcp.as_object_mut().unwrap().insert(ENTRY_NAME.to_string(), entry);
...
fn install_json(path: &Path, key: &str, entry: Value) -> Result<(), String> {
    ...
    servers.as_object_mut().unwrap().insert(ENTRY_NAME.to_string(), entry);
    write_json(path, &root)
}
...
mod tests {
    ...
    fn new_clients_preserve_other_settings_across_transport_changes_and_removal() {
        for (c, url_key) in [(McpClient::Antigravity, "serverUrl"), (McpClient::OhMyPi, "url")] {
            let dir = tempfile::tempdir().unwrap();
            let path = client_path_in(dir.path(), c);
            write_json(&path, &json!({"preferences": {"keep": true}, "mcpServers": {"other": {"command": "keep"}}})).unwrap();
            install_at(c, &path, &automatic_settings(c, &settings(McpTransport::Http, false))).unwrap();
            ...
            if matches!(c, McpClient::OhMyPi) { assert_eq!(entry["type"], "http"); }
            install_at(c, &path, &automatic_settings(c, &settings(McpTransport::Http, true))).unwrap();
            let root = read_json(&path).unwrap();
            let entry = &root["mcpServers"]["tucano"];
    ...
    fn automatic_connection_respects_client_and_autolaunch() {
        for c in CLIENTS {
            ...
            assert_eq!(manual.transport, if matches!(c, McpClient::ClaudeDesktop) { McpTransport::Stdio } else { McpTransport::Http });
    ...
    fn gemini_transport_swap_preserves_other_settings_and_servers() {
        ...
        write_json(&path, &json!({"theme": "custom", "mcpServers": {"other": {"command": "other"}}})).unwrap();
        install_at(McpClient::Gemini, &path, &settings(McpTransport::Http, false)).unwrap();
        let root = read_json(&path).unwrap();
        assert_eq!(root["mcpServers"]["tucano"]["httpUrl"], "http://127.0.0.1:7878/mcp");
        assert_eq!(root["mcpServers"]["tucano"]["headers"]["Authorization"], "Bearer test-token");
        install_at(McpClient::Gemini, &path, &settings(McpTransport::Stdio, true)).unwrap();
        let root = read_json(&path).unwrap();
        assert!(root["mcpServers"]["tucano"].get("httpUrl").is_none());
        assert_eq!(root["mcpServers"]["tucano"]["env"]["TUCANO_MCP_AUTOLAUNCH"], "1");
    ...
    fn codex_desktop_and_cli_share_installation() {
        ...
        assert_ne!(client_meta(McpClient::CodexDesktop).0, client_meta(McpClient::Codex).0);
        install_at(McpClient::CodexDesktop, &desktop, &settings(McpTransport::Http, false)).unwrap();
    ...
    fn grok_install_uninstall_http() {
        ...
        assert!(entry.get("http_headers").is_none(), "Grok key must be `headers`, not `http_headers`");
    ...
    fn grok_install_uninstall_stdio() {
        ...
        assert_eq!(
            entry["args"].as_array().unwrap().iter().map(|v| v.as_str().unwrap()).collect::<Vec<_>>(),
            vec!["mcp-stdio"]
        );
        let env = entry["env"].as_inline_table().unwrap();
        assert_eq!(env.get("TUCANO_MCP_URL").unwrap().as_str().unwrap(), "http://127.0.0.1:7878/mcp");
        assert_eq!(env.get("TUCANO_MCP_TOKEN").unwrap().as_str().unwrap(), "test-token");
        assert_eq!(env.get("TUCANO_MCP_AUTOLAUNCH").unwrap().as_str().unwrap(), "1");
    ...
    fn grok_stdio_without_autolaunch_omits_key() {
        let dir = tempfile::tempdir().unwrap();
        let path = client_path_in(dir.path(), McpClient::Grok);
        install_at(McpClient::Grok, &path, &settings(McpTransport::Stdio, false)).unwrap();
        let doc = read_toml(&path).unwrap();
        let env = doc["mcp_servers"]["tucano"]["env"].as_inline_table().unwrap();
    ...
    fn codex_transport_swap_http_stdio_http_leaves_no_leftovers() {
        ...
        install_at(McpClient::Codex, &path, &settings(McpTransport::Http, false)).unwrap();
        install_at(McpClient::Codex, &path, &settings(McpTransport::Stdio, false)).unwrap();
        {
            let doc = read_toml(&path).unwrap();
            let entry = doc["mcp_servers"]["tucano"].as_table().unwrap();
            assert!(entry.get("url").is_none(), "url must not survive switching to stdio");
            assert!(entry.get("http_headers").is_none(), "http_headers must not survive switching to stdio");
            assert!(entry.get("command").is_some());
        }
        ...
        install_at(McpClient::Codex, &path, &settings(McpTransport::Http, false)).unwrap();
        {
            let doc = read_toml(&path).unwrap();
            let entry = doc["mcp_servers"]["tucano"].as_table().unwrap();
            assert!(entry.get("command").is_none(), "command must not survive switching back to http");
    ...
    fn is_installed_detects_both_json_formats() {
        ...
        install_at(McpClient::ClaudeCode, &path, &settings(McpTransport::Http, false)).unwrap();
        assert!(is_installed(McpClient::ClaudeCode, &path));
        install_at(McpClient::ClaudeCode, &path, &settings(McpTransport::Stdio, false)).unwrap();
        assert!(is_installed(McpClient::ClaudeCode, &path));
    }
    ...
    fn old_settings_file_without_new_fields_loads_with_defaults() {
        ...
        .unwrap();
        let s = McpSettings::load(dir.path());
        assert!(s.enabled);
    ...
    fn prints_grok_config_toml_http_and_stdio() {
        ...
        install_at(McpClient::Grok, &path, &settings(McpTransport::Http, false)).unwrap();
        println!("--- Grok config.toml (http) ---\n{}", std::fs::read_to_string(&path).unwrap());

        install_at(McpClient::Grok, &path, &settings(McpTransport::Stdio, true)).unwrap();
        println!("--- Grok config.toml (stdio) ---\n{}", std::fs::read_to_string(&path).unwrap());
    }
}
