...
static PORT_CACHE: Lazy<Mutex<HashMap<u16, (Instant, ClientInfo)>>> =
    Lazy::new(|| Mutex::new(HashMap::new()));
...
fn store(port: u16, info: &ClientInfo) {
    PORT_CACHE
        .lock()
        .insert(port, (Instant::now(), info.clone()));
}
...
mod macos {
    ...
    pub fn resolve(port: u16) -> ClientInfo {
        let Some(pid) = pid_for_port(port) else {
    ...
    fn icon_for_bundle(bundle: &str) -> Option<String> {
        if let Some(cached) = ICON_CACHE.lock().get(bundle).cloned() {
            return cached;
        }
        ...
        ICON_CACHE
            .lock()
            .insert(bundle.to_string(), Some(data_url.clone()));
        Some(data_url)
    }
    ...
    fn find_icns(resources: &str, bundle: &str) -> Option<String> {
        ...
        if let Ok(o) = Command::new("defaults")
            .args(["read", &plist, "CFBundleIconName"])
            .output()
        {
            if o.status.success() {
                let raw = String::from_utf8_lossy(&o.stdout).trim().to_string();
...
mod windows {
    ...
    fn process_name(pid: u32) -> Option<String> {
        ...
        s.lines().next().and_then(|l| {
            ...
            parts
                .first()
                .map(|n| n.trim_matches('"').trim_end_matches(".exe").to_string())
        })
    }
}
