...
/// Start only after binding succeeds; callers can rely on readiness.
pub async fn spawn(state: Arc<AppState>, port: u16, token: String) -> Result<(), String> {
    let _guard = state.mcp_lifecycle.lock().await;
    stop_unlocked(&state).await;
    spawn_unlocked(state.clone(), port, token).await
}
pub(crate) async fn spawn_unlocked(
    state: Arc<AppState>,
    port: u16,
    token: String,
) -> Result<(), String> {
    ...
    *state.mcp_stop_tx.lock() = Some(tx);
    let st = state.clone();
    *state.mcp_task.lock() = Some(tokio::spawn(async move {
        if let Err(e) = run(st.clone(), listener, token, rx).await {
            let _ = st.emit("mcp:error", e.to_string());
...
pub(crate) async fn stop_unlocked(state: &Arc<AppState>) {
    if let Some(tx) = state.mcp_stop_tx.lock().take() {
        let _ = tx.send(());
    }
    let task = state.mcp_task.lock().take();
...
pub async fn handle_rpc(state: &Arc<AppState>, msg: Value) -> Option<Value> {
...
fn tool_status(state: &Arc<AppState>) -> Result<Value, String> {
    let count = state.storage.lock().count().map_err(|e| e.to_string())?;
    Ok(json!({
...
fn tool_stats(state: &Arc<AppState>) -> Result<Value, String> {
    ...
    slowest.sort_by_key(|flow| std::cmp::Reverse(flow.duration_ms));
    ...
    largest.sort_by_key(|flow| std::cmp::Reverse(flow.res_size));
...
fn tool_diff(state: &Arc<AppState>, args: Value) -> Result<Value, String> {
    ...
    let (fa, fb) = {
        ...
        let fa = s
            .get(a)
            .map_err(|e| e.to_string())?
            .ok_or("flow a not found")?;
        let fb = s
            .get(b)
            .map_err(|e| e.to_string())?
            .ok_or("flow b not found")?;
        (fa, fb)
    };
...
fn tool_delete(state: &Arc<AppState>, args: Value) -> Result<Value, String> {
    let ids: Vec<String> = args
        ...
        .unwrap_or_default();
    crate::commands::delete_flows(state.clone(), ids.clone())?;
    Ok(json!({ "deleted": ids.len() }))
}

fn tool_clear(state: &Arc<AppState>) -> Result<Value, String> {
    crate::commands::clear_flows(state.clone())?;
...
async fn tool_replay(state: &Arc<AppState>, args: Value) -> Result<Value, String> {
    ...
    let new_id =
        crate::commands::send_request(state.clone(), flow, headers, body.body, "MCP Replay")
            .await?;
    Ok(json!({ "id": new_id }))
}
...
async fn tool_compose(state: &Arc<AppState>, args: Value) -> Result<Value, String> {
    ...
    let flow =
        crate::commands::compose_internal(state.clone(), method, url, headers, body, log).await?;
...
fn tool_set_ssl(state: &Arc<AppState>, args: Value) -> Result<Value, String> {
    ...
    crate::commands::set_ssl_settings(state.clone(), settings.clone())?;
...
fn fetch_flows(state: &Arc<AppState>, ids: &[String]) -> Result<Vec<crate::storage::Flow>, String> {
    let s = state.storage.lock();
    ids.iter()
        .map(|id| {
            s.get(id)
...
fn skip_header(k: &str) -> bool {
    ...
    matches!(
        k.as_str(),
        "content-length" | "host" | "transfer-encoding" | "connection" | "proxy-connection"
    )
}
...
fn body_as_text(f: &crate::storage::Flow) -> Option<&str> {
    if f.req_body_encoding == "base64"
        || f.req_truncated
        || f.req_body.as_ref().is_some_and(|b| b.contains("[REDACTED"))
    {
...
pub(crate) fn flow_to_curl(f: &crate::storage::Flow, include_headers: bool) -> String {
    let mut lines = vec![format!(
        "curl -X {} {}",
        shell_quote(&f.method),
        shell_quote(&flow_url(f))
    )];
    if include_headers {
        for (k, v) in &f.req_headers {
            if skip_header(k) || v.contains("[REDACTED") {
                continue;
            }
            lines.push(format!("  -H {}", shell_quote(&format!("{k}: {v}"))));
        }
    }
    let binary = f.req_body.as_deref().filter(|body| {
        f.req_body_encoding == "base64"
            && !f.req_truncated
    ...
    if let Some(body) = body_as_text(f) {
        ...
        lines.push("  --data-binary @-".into());
    }
    let incomplete = f.req_truncated
        || f.req_headers.iter().any(|(_, v)| v.contains("[REDACTED"))
        || f.path.to_ascii_uppercase().contains("%5BREDACTED")
        || f.req_body
            .as_ref()
            .is_some_and(|_| body_as_text(f).is_none() && binary.is_none());
    let mut command = lines.join(" \\\n");
    if let Some(body) = binary {
        command = format!("printf '%s' {} | base64 -d | {command}", shell_quote(body));
    }
    if incomplete {
        format!("# Redacted credentials or incomplete body omitted; supply replacements before replay.\n{command}")
    } else {
        command
    }
}
...
fn flow_to_code(f: &crate::storage::Flow, lang: &str) -> String {
    let url = flow_url(f);
    let headers =
        serde_json::to_string_pretty(&header_object(f)).unwrap_or_else(|_| "{}".to_string());
...
pub(crate) fn flow_to_har(flows: &[crate::storage::Flow]) -> Value {
...
pub(crate) fn tools_list() -> Value {
    let pair =
        json!({ "type": "array", "items": { "type": "string" }, "minItems": 2, "maxItems": 2 });
    json!([
        {
            "name": "tucano_status",
