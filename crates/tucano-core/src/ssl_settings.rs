...
pub struct SslSettings {
    ...
    /// Exact development hosts allowed to use untrusted upstream certificates.
    /// Empty by default. Wildcards are deliberately not accepted.
    #[serde(default)]
...
impl SslSettings {
    pub fn load(dir: &Path) -> crate::state::BoxResult<Self> {
        match std::fs::read_to_string(dir.join("ssl-settings.json")) {
...
mod tests {
    ...
    fn pjeoffice_app_is_bypassed_by_default() {
        ...
        assert!(
            s.should_skip_app("java -jar /Applications/pjeoffice-pro.app/.../pjeoffice-pro.jar")
        );
