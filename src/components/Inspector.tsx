...
export default function Inspector({ flow, onClose, onComposer }: { flow: Flow | null; onClose?: () => void; onComposer?: (flow: Flow) => void }) {
  ...
  return (
    <div data-inspector="true" className="h-full flex flex-col relative bg-white dark:bg-[#080D1B]">
      {!flow ? (
        ...
        <>
          ...
          {showTiming ? (
            <div className="flex-1 overflow-auto scroll-thin bg-white dark:bg-[#080D1B]">
...
function Pane({ label, accent, tab, onTab, focused, onToggleFocus, onHide, children }: {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-auto scroll-thin bg-white dark:bg-[#080D1B]">
        {children}
      </div>
    </div>
  );
