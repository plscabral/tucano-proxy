              ...
              <td class="px-5 py-2.5 align-top break-all whitespace-pre-wrap relative">
                <div class="flex gap-2">
                  ...
                  <span class="flex gap-1 opacity-0 group-hover:opacity-100 transition shrink-0 self-start sticky top-0">
                    <button
                      onClick={() => copy(valId, v)}
                      title="Copy value"
                      class="h-6 px-2 rounded-md text-[10px] flex items-center gap-1 bg-white dark:bg-ink-500 border border-ink-100 dark:border-ink-400/40 hover:border-toucan-400 hover:text-toucan-400"
                    >
                      {copied() === valId ? <Check size={11} /> : <Copy size={11} />} value
                    </button>
                    <button
                      onClick={() => copy(lineId, `${k}: ${v}`)}
